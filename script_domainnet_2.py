...
def get_motion_blur_kernel(size, angle=0):
    ...
    Args:
        size: kernel size (odd number recommended)
        angle: motion direction in degrees (0=horizontal, 90=vertical)
    
    Returns:
        2D numpy array representing motion blur kernel
    """
...
def apply_motion_blur_to_image(img, kernel_size, angle=45, mode="reflect", cval=0):
    """
    Apply motion blur to a single image
    
    Args:
        img: numpy array of shape (H, W, 3) or (H, W)
        kernel_size: size of motion blur kernel
        angle: motion direction in degrees
        mode: how to handle borders ('constant', 'reflect', 'nearest', 'mirror', 'wrap')
    ...
    if img.ndim == 2:
        # Grayscale
        blurred = convolve(img.astype(np.float32), kernel, mode=mode, cval=cval)
        blurred = np.clip(blurred, 0, 255).astype(img_dtype)
    else:
        # Color
        blurred = np.zeros_like(img, dtype=np.float32)
        for c in range(img.shape[2]):
            blurred[..., c] = convolve(img[..., c].astype(np.float32), kernel, mode=mode, cval=cval)
        blurred = np.clip(blurred, 0, 255).astype(img_dtype)
...
def motion_blur_dataset_h5(
    ...
    """
    Apply motion blur to train and test datasets in an HDF5 (.h5) file.
    ...
    plus any optional extra datasets (e.g. train_impaths, test_impaths).

    The output HDF5 will have the same datasets/attributes, except that
    train_data and test_data are replaced by their motion-blurred versions.
    """
    ...
    with h5py.File(input_h5_path, "r") as src, h5py.File(output_h5_path, "w") as dst:
        ...
        train_data_src = src["train_data"]
        test_data_src  = src["test_data"]

        # Preserve dtype & shape
        train_shape = train_data_src.shape
        test_shape  = test_data_src.shape
        train_dtype = train_data_src.dtype
        test_dtype  = test_data_src.dtype

        print(f"Train data: {train_shape}, dtype: {train_dtype}")
        print(f"Test data:  {test_shape}, dtype: {test_dtype}")

        # Create blurred datasets
        train_data_dst = dst.create_dataset("train_data", shape=train_shape, dtype=train_dtype)
        test_data_dst  = dst.create_dataset("test_data",  shape=test_shape,  dtype=test_dtype)

        # Copy labels directly (they are small)
        for lbl_name in ["train_label", "test_label"]:
            if lbl_name in src:
                dst.create_dataset(lbl_name, data=src[lbl_name][...])
            else:
                print(f"Warning: '{lbl_name}' not found in input; skipping.")

        # Copy any additional datasets unchanged (e.g. train_impaths, test_impaths)
        for name in src.keys():
            if name in ["train_data", "test_data", "train_label", "test_label"]:
                continue
            src.copy(name, dst)

        # --- apply blur to train_data ---
        print(f"\nApplying motion blur to training data (kernel_size={kernel_size_train}, angle={angle_train}°)...")
        n_train = train_shape[0]
        for i in tqdm(range(n_train), desc="Train"):
            img = train_data_src[i]          # slice from HDF5 -> numpy array
            blurred = apply_motion_blur_to_image(
                img,
                kernel_size_train,
                angle_train,
                mode=mode,
            ...
            train_data_dst[i] = blurred

        # --- apply blur to test_data ---
        print(f"\nApplying motion blur to test data (kernel_size={kernel_size_test}, angle={angle_test}°)...")
        n_test = test_shape[0]
